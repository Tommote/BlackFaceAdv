...
class LFWImagePairList():
    def __init__(self, root_path='/home/srq/datasets/LFW/lfw_all/' , transform=None, is_gray=False, alignment=True, out_shape=(96,112)):
        """
        ...
        Return: a pair of image, and its type is torch.tensor [1, 3, out_shape[1], out_shape[0]]
        ...
        self.landmark = {}
        with open('src/utils/lfw_landmark.txt') as f:
    ...
    def __getitem__(self, index):
        ...
        if self.gray:
            img1 = cv2.imread(self.root_path+name1 , cv2.IMREAD_GRAYSCALE)
            img2 = cv2.imread(self.root_path+name2 , cv2.IMREAD_GRAYSCALE)
        else:
            img1 = cv2.imread(self.root_path+name1 , cv2.IMREAD_UNCHANGED)
            img2 = cv2.imread(self.root_path+name2 , cv2.IMREAD_UNCHANGED)
        ...
        # print(img1.shape)
        img1, img1_ = self.transform(img1), self.transform(F.hflip(img1))
        img2, img2_ = self.transform(img2), self.transform(F.hflip(img2))
...
class LFWImageAlignPairList():
    def __init__(self, root_path='/home/srq/datasets/LFW/lfw-align-128/' , transform=None, is_gray=True):
        """
        This is the Dataset of LFW, the original image will be precoessed including 
        alignment , and will return a pair of image such as (), ()
        ...
        Return: a pair of image, and its type is torch.tensor [1, 1, 128, 128]
                (img1, img1_),(img2, img2_)
        """
        ...
        with open('src/utils/lfw_test_pair_new.txt') as f:
            self.pairs_lines = f.readlines()
        

    def __getitem__(self, index):
        
        spl = self.pairs_lines[index].split()
